...
const VARIANT_TO_BUTTONS_KEY: Partial<
  Record<ButtonVariant, ButtonTokensKey>
> = {
  primary: 'primary',
  secondary: 'secondary',
  brand: 'brand',
...
export const Button = forwardRef<ViewType, ButtonProps>(function Button(
  ...
  if (
    ...
    console.warn(
      `[chilli-native] <Button glass> is not supported for variant="${variant}". Rendering as non-glass.`,
    );
  }
  ...
  const textColor = getTextColor(variant, isDisabled, isGlassActive);
        ...
        <>
          ...
          <View style={styles.content}>
            {loading ? (
              ...
              <LeftIcon size={iconSize} color={textColor} />
            ) : null}
            {children != null ? (
              <Text
                variant={size === 'xsm' || size === 'sm' ? 'bodySm' : 'bodyMd'}
                color={textColor}
              >
                {children}
              </Text>
            ...
            {RightIcon && !loading ? (
              <RightIcon size={iconSize} color={textColor} />
            ) : null}
          </View>
        </>
      )}
...
const SIZE_STYLES = StyleSheet.create<Record<ButtonSize, ViewStyle>>({
  xsm: {
    height: 28,
    paddingHorizontal: 8,
    gap: 0,
