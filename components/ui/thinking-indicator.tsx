...
export const ThinkingIndicator = forwardRef(({
  ...
  return <div ref={ref} role="status" className={cn("flex items-center gap-2 px-3 py-2", className)} {...props}>{<motion.svg aria-hidden width={20} height={20} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={1.5} strokeLinecap="round" strokeLinejoin="round" className="text-[var(--text-base-secondary)] shrink-0">{<motion.path animate={{
      ...
      }} />}</motion.svg>}{<span className="inline-grid text-[13px] font-medium overflow-hidden">{<span className="col-start-1 row-start-1 invisible shimmer-text" aria-hidden="true">{longest}</span>}{<AnimatePresence mode="popLayout" initial={false}>{<motion.span className="col-start-1 row-start-1 shimmer-text" initial={{
          y: "80%",
          opacity: 0
        }} animate={{
          y: 0,
        ...
        }}>{words[index]}</motion.span>}</AnimatePresence>}</span>}</div>;
});
