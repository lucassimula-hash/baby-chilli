...
export function InputFieldDemo() {
  ...
  return <div className="flex flex-col gap-6 w-full max-w-[400px]">{<Input label="Location" placeholder="Add a city" helperText="Used to personalize nearby actions." leftIcon={<MapPin size={20} />} rightIcon={<ChevronRight size={20} />} />}{<Input label="Email" value={value} onChange={e => setValue(e.target.value)} placeholder="Enter your email" leftIcon={<Mail size={20} />} clearable onClear={() => setValue("")} helperText="We'll never share your email." />}{<Input label="Website" leadingText="https://" placeholder="your-site.com" rightIcon={<ChevronRight size={20} />} helperText="Enter your website URL" />}{<Input label="Username" value="ab" error="Username must be at least 3 characters." leftIcon={<Search size={20} />} />}{<Input label="Disabled field" placeholder="Can't edit this" disabled />}</div>;
}
export function ActionInputDemo() {
  const [toggleVal, setToggleVal] = useState(false);
  return <div className="flex flex-col gap-6 w-full max-w-[400px]">{<ActionInput label="Location" placeholder="Add a city" leftIcon={<MapPin size={20} />} helperText="Used to personalize nearby actions." />}{<ActionInput label="Location" defaultValue="Paris, France" leftIcon={<MapPin size={20} />} helperText="Used to personalize nearby actions." />}{<ActionInput label="Assigned to" defaultValue="@username" showAvatar avatarSrc="https://i.pravatar.cc/40" helperText="Tag the owner of this action." />}{<ActionInput label="Notifications" defaultValue="Push enabled" showToggle toggleChecked={toggleVal} onToggleChange={setToggleVal} rightIcon={null} helperText="Enable push updates for new activity." />}{<ActionInput label="Location" defaultValue="Pa" leftIcon={<MapPin size={20} />} error="Enter at least 3 characters." />}</div>;
}
